# 第 7 课：Agent 执行安全——审批、权限与 Sandbox
...
## 2. Approval 与 Permission：审批和进程权限
...
`ready` 只表示通过了这两项应用检查。**Permission（进程权限）** 指进程实际拥有的访问能力：如果操作系统不允许它读文件，界面上点“允许”仍然读不开。审批按钮不会把进程变成 root。
...
有些 Agent 允许某次命令离开平常的沙盒，改到宿主执行环境，称为 **Elevated**。这里说的是执行环境变化，不等于获得管理员权限，也不是所有产品统一使用的名称。
...
`allow_elevated` 来自可信策略。更高层要求必须隔离时，它就不能为真，模型无法自己开通权限。允许一次命令在宿主执行环境运行，也不应让后续命令永久离开沙盒。

后端选好后，还可能创建失败。如果本次操作要求强制隔离，创建失败就必须停止，不能偷偷改到宿主执行环境重跑。这种“保护条件不成立就拒绝执行”的规则叫 **Fail-closed**。安全设施坏了，不能用“先裸跑一下”提高可用性。
...
| 插件、Hook、MCP 子进程 | 是否也进入隔离，还是留在宿主执行环境 |
...
[进入第 7 课实践](../practice/lesson-07/README.md)：对照允许与拒绝结果，解释审批、工作目录和系统隔离的边界。说明中列出必做步骤、完成标准和选做内容。
...
4. [项目对照、固定源码与核验记录](../research/07-safety-source-verification.md)
5. 本机命令手册：`man sandbox-exec`。本轮核验了 `-p` 用法与弃用标记；该实验不替代生产沙盒的选型和安全验证。

...
![完整流水保留，快照记录前两笔的余额 7，小黑只处理第三笔加 5，恢复为 12](../assets/session-persistence-illustrations/01-transcript-checkpoint-resume.png)

快照已计入前两笔，恢复从余额 7 接着处理第三笔加 5。完整流水仍保留；这里重建的是状态，不是重做历史动作。
...
[进入第 4 课实践](../practice/lesson-04/README.md)：保存并读回会话，增加一条消息，核对历史与快照位置。说明中列出必做步骤、完成标准和选做内容。
...
3. [第 4～5 课相关资料核验](../research/01-05-chapter-promotion-sources.md)

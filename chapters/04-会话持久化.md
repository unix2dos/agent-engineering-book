...
你花了一个下午，耐心地向运维 Agent 传授你们团队祖传的二十条线上部署军规：“千万别删 `/data`”、“先关网关再启服务”、“部署博客前必须运行 `hexo g`”。

Agent 语气沉稳地答道：“收到，所有军规均已牢记在心，请您放心。”

接着你打了个哈欠，随手关掉终端去泡了杯咖啡。十分钟后重新开机，你随口问它：“部署前第一件事是什么？”它眨了眨眼，像面对一位素未谋面的陌生人一样礼貌回应：“您好！我是您的智能助手，请问有什么可以帮您？”

退出进程就是模型的赛博脑死亡。不管前一秒它在对话中表现得多像一个十年老兵，一旦内存清空，它的认知就彻底归零。如果对话只保存在运行时的变量里，程序一关，`messages` 就消失了。
...
第 5 课会补上缺失的那一半：改用追加式的 `session-demo.jsonl` 保存完整事件流，再在此基础上按需组装有限的 Prompt View。其中写入的 `type="compaction"` 记录会保存历史摘要和保留原文，作为 Transcript 内部的压缩恢复点——这本身就是 Checkpoint 的另一种存在形态。

## 8. Checkpoint 为什么不能证明 Tool 是否执行过？

教学代码只在 Assistant Final 出现后保存完整 Turn。假设外部动作（如写文件、发请求）已经发生，但程序还没来得及记录 Tool Result 就意外崩溃：**外部世界已经被改变，而 Checkpoint 却仍停留在上一个完整状态。**

重启后看到的旧 Checkpoint，只能说明“程序最后成功存盘到了哪里”，绝不能证明外部动作到底执行了几次。

这类动作需要单独的执行收据，记录何时获准、何时开始、最后确认成功还是状态不明。后面的可靠性课程会把这份收据叫作 Execution Ledger，并讨论怎样避免重复执行。

## 9. JSONL 写到十万行，什么时候该换 SQLite？

假设 `session.jsonl` 已经写了十万行。程序重启后，你想找出所有最终状态仍是 `unknown` 的工具执行。这里的 `unknown` 是说：工具可能已经改变了外部世界，但本地程序还无法确认结果。第 6 课会专门处理这种故障。
...
只搜索 `unknown` 会错把 `exec_2` 也找出来。程序必须从第一行读到最后一行，再按 `execution_id` 整理每次执行的最新状态。

JSONL 没有坏。变化的是程序使用它的方式：原来只需追加和偶尔回放，现在每次启动都要按字段查询、合并状态，甚至让多个进程同时写入。

决定是否更换存储的关键不是文件大小，而是平时怎样读写：

| 实际需求 | 合适的起点 |
| --- | --- |
| 保存一份最新 Checkpoint，修改时整体替换 | JSON |
| 单进程按顺序追加 Transcript，偶尔从头回放 | JSONL |
| 同一台机器频繁查询，需要事务或唯一约束 | SQLite |
| 多台机器共同写入和查询 | PostgreSQL 等服务端数据库 |
| 只想续接 Provider 托管的模型对话 | Server-managed Conversation |

十万行只在归档时读取一次，JSONL 可能仍然够用；一万行却在每次启动时反复整理，SQLite 可能已经更省事。

SQLite 也不会改变数据本身的职责。换成数据库后，Transcript 仍回答“发生过什么”，Checkpoint 仍回答“程序走到哪里”，Memory 仍保存跨 Session 的稳定事实。存储介质只是让某些查询和一致性约束更容易实现。

想亲手观察这个变化，可以运行 [SQLite 专项练习](../exercises/session-storage-sqlite/README.md)。它会从“扫描事件找当前状态”开始，再验证事务和唯一约束。那些机制为什么会影响 Tool 重试，要到第 6 课放进真实副作用场景后才完整。
...
- [存储 Backend 一手资料复核](../research/storage-backend-source-verification.md)

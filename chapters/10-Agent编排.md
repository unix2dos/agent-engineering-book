...
配套流程使用预设模型回复：第一轮写错端口，收到程序反馈后，第二轮读取当前文件并改回 3000。文件工具、Ledger 和评分器真实执行，两轮产物分别留存。
...
已有一次真实运行验证了另一条停止路径：模型读写完成，文件评分 passed，最后的模型回答却出现 `APITimeoutError`。程序保存成功写入的 Ledger，整体停为 run_error，没有自动开始下一轮。
...
写下一个任务编号，是继续查询的依据，不是远端任务已经完成的证明。
...
两者的层次关系第 2 课已经说明。放到这个任务里，选哪个的区别在于采用现成的 Agent 组织方式，还是直接控制步骤；不是“单 Agent 对多 Agent”。[3][4]
...
能解释一次任务为什么继续、等待或停止，比先给它安排几个 Agent 更重要。
...
配置工作流已经接通：固定剧本验证了接线与分支，不证明真实模型一定能修好。预算在单进程内扣减，扣减与请求发出之间仍有故障窗口，没有验证并发原子扣减；写了 Checkpoint 也不等于实现了自动续跑。第 2 节的 `APITimeoutError` 是一次历史观察，不是成功率评测，也没有验证模型收到错误反馈后会修复；实测条件、结果与源码指纹见本课实践说明链接的历史记录。

交接与子任务恢复是独立机制实验：由一个进程保存状态、另一个进程读取，下游结果来自预设记录；它验证了复用、等待和核对决定，没有启动真实子 Agent、并行调度或执行合并。配套代码没有实现远端取消或通用后台续跑。

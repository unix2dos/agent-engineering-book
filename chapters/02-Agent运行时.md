...
LangChain 与 LangGraph 都是可以安装到程序中的代码库，能承担一部分 Harness 工作。
...
做排查助手时，把模型、读取工具和任务要求交给 LangChain 的 Agent，它可以组织“请求模型—执行工具—回传结果”的循环。文件具体怎样读取，仍由工具实现。[5]

LangGraph 用来连接任务步骤，安排接下来去哪，并可按配置保存运行进度。图里的底座和抽屉表示这些能力，不是两台独立设备，也不代表故障恢复已经自动做好。[6]

LangChain 的 Agent 建立在 LangGraph 之上；直接使用 LangGraph，也不要求先使用 LangChain。[5][6]

现阶段先记住它们承担什么职责。等[第 10 课](10-Agent编排.md)真正遇到“修改—验收—返工”时，再完整解释节点、分支和状态怎样对应到代码。

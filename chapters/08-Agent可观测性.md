...
线上监控报警响了，你的 Agent 任务在一声刺耳的“Execution Failed”中暴毙。

你心急如焚地分头排查日志，却看到了一场各执一词的赛博罗生门：

```text
Model 日志：请求耗时 1.2s，HTTP 200，Token 已扣费
Tool 日志：命令执行耗时 0.05s，exit code 0，完美收工
应用日志：系统在空等三分钟后判定超时，连接强行断开
```

每个人都拿着一张无罪证明，每个人都在宣称自己尽职尽责，但你的用户就是没有拿到结果。

这些记录确实没有说谎，但它们各自只拍到了大象的一小段皮肤。Model 的请求属于哪次任务？Tool 是由哪次 Model 请求发起的？失败发生在 Tool 前面，还是最终回答后面？如果只能在混乱的时间戳里靠肉眼连连看，日志越多，真相反而越难拼出来。
...
下一课研究[怎样评估 Agent 改动并制定回归门禁](09-Agent评估.md)：哪些任务值得选入测试集，怎样检查真实结果，以及什么证据支持放行。Trace 可以帮助解释失败；即使尚未接入正式 Tracing，也能先根据实际产物、会话和执行记录开展评估。

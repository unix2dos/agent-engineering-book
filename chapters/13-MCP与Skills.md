...
[本地 MCP 对照实验](../practice/optional/mcp/README.md)保留了直接调用与跨进程调用的代码。2026-09-16 已修复并复核结构化输出约定：family、personal 分别返回账号上限 6、3，无效套餐和不存在的写工具均返回错误。实验验证本地工具发现、跨进程调用与错误处理，尚未接入真实模型；版本和检查条件见实践说明。

正文缩小示例与完整对照脚本是两份不同尺度的说明，实际实验以该入口为准。沿图解释工具服务接入后，可以继续到[第 14 课的运行控制](14-Agent运行控制.md)；MCP 不会自动提供共享并发、任务预算或资源回收策略。

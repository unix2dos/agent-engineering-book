...
回到本例，整套客服助手叫 **Host（宿主应用）**，其中负责运行控制的部分是 Harness。Host 内负责与工具服务通信的组件叫 **MCP Client**，提供查询能力的程序叫 **MCP Server**。它们可以运行在同一台电脑上；Client 通常是 Host 的一部分，不是又一台需要部署的机器。[1]
...
[本地 MCP 对照实验](../practice/optional/mcp/README.md)保留了直接调用与跨进程调用的代码。当前已观察到初始化、工具发现和 family 查询成功返回 6，但结果检查因假定存在 `structuredContent` 而失败；后续无效参数与未知写工具检查尚未执行，不能称为整套实验通过，也没有接入真实模型。

正文缩小示例已验证函数结果与工具登记，并明确返回类型为 `dict[str, int]`。它与原对照脚本不是同一份实现，不代表原实验已修复。当前先把调用过程讲清楚，后续修复再补完整实测结果。

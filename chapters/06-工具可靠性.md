...
![恢复程序对照已写着 done 的 diagnosis.txt 与最后停在 running、未记录结果的 Ledger](../assets/tool-reliability-illustrations/01-effect-without-receipt.png)

图中是写入已完成、回执未记录的情况；账本里的 running 是旧进程最后留下的记录。中断也可能发生在写入前，恢复时要核对证据，不能只凭缺少回执就重做。
...
[进入第 6 课实践](../practice/lesson-06/README.md)：模拟写入后中断，核对账本与文件，验证恢复不会盲目重写。说明中列出必做步骤、完成标准和选做内容。
...
3. [项目对照、固定源码及核验记录](../research/06-tool-reliability-source-verification.md)

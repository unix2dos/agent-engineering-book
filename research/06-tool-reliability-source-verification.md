...
第 6 课的主线成立：Assistant Tool Call、真实执行尝试和模型可见 Tool Result 是三件事。程序在副作用完成后、Tool Result 持久化前崩溃时，恢复系统必须依靠执行记录和外部证据，不能仅凭 Session 猜测或直接重跑。

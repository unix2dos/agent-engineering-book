...
第 15 课 Computer Use 是界面操作专题，沿用工具循环与执行边界，新增界面目标定位、浏览器接入、登录环境和桌面范围的判断。
...
| 15 Computer Use（专题） | 用户目标怎样定位到界面对象，工具究竟控制哪个环境？ | 3、7；恢复与限额按需回看 6、14 | 解释截图、DOM、AX 的定位依据，区分库、扩展、CDP 与登录环境，复盘真实操作结果 | [正文末尾](../chapters/15-ComputerUse.md) |
...
- **正文状态**：文章是否成稿，是否进入正式目录。当前正文目录为第 0～15 课，第 15 课是新增的界面操作专题。
...
第 15 课已有通过现有工具完成的浏览器只读演示。BrowserSkill、Playwright MCP 和 Cua 的比较来自文档与源码核验；没有安装运行整套项目，也没有完成桌面隔离或提示注入防护实验。
...
浏览器与桌面操作已进入第 15 课专题。大规模 Multi-Agent、完整观测平台、高可用调度、A2A 和其他多模态能力仍按真实需求进入，不阻塞当前主线。阶段结束后的面试表达应基于已完成的项目证据，不能反过来把正文扩成题库。

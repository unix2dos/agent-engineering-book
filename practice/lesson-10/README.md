...
工具次数额度与预留总结的深入练习，统一从[第 14 课实践](../lesson-14/README.md)进入；仍复用本目录的工作流代码。

## 完成标准

能根据每轮产物、评分和剩余额度说明继续或停止的理由。必做改动是将 `--max-attempts` 改为 `1`，或将 `--model-budget` 改为 `5`；文件通过但完整运行未结束时，不能宣布任务成功。
...
2026-09-10 的一次真实运行中，文件通过验收，但最后的模型回答超时，完整运行仍为 `run_error`。程序停止并保留成功的写入回执，没有自动重新写文件。实验过程见[真实验证记录](live-check.md)。
...
```bash
python -B practice/lesson-10/check.py
```
...
python -B practice/lesson-10/handoff_demo.py --self-check
python -B practice/lesson-10/recovery_demo.py --self-check
```

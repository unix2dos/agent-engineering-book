...
本文是 2026-09-10 的历史实测记录。2026-09-15 仅迁移位置；下方旧文件名与指纹仍描述当次运行，未重新调用真实模型。当前运行入口见[第 10 课实践](README.md)。
...
后续应先明确怎样继续已有会话、沿用剩余预算和已有写入结果；不能简单重新执行整个工作流。当次实验尚未实现这一恢复入口；后续行为以当前实践说明为准。

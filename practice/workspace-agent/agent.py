"""完成版 Workspace Agent：工具循环、会话、执行账本与恢复；文件后半为离线检查。"""
...
def make_idempotency_key(tool_name: str, tool_call_id: str) -> str:
    """第四关 B。为同一个 Tool Call 生成稳定幂等 Key。"""
...
def append_execution_state(
    ...
    """第四关 A。追加一条 Tool Execution Ledger Entry。"""
...
def mark_interrupted_executions_unknown(session_file: Path) -> list[str]:
    """第五关 B。把重启时遗留的 running 追加为 unknown。"""
...
def repair_missing_tool_results(session_file: Path) -> list[str]:
    """第五关 C。根据 Ledger 补写缺失的 Tool Result。"""
...
def reconcile_unknown_write_files(
    ...
    """第五关 E。核对 unknown write_file 的目标状态。"""
    entries = load_entries(session_file)
    calls = pending_tool_calls(entries)
    states = latest_execution_by_tool_call(entries)
    reconciled = []
...
def maybe_compact(
    ...
    if not summary.strip():
        raise ValueError("Compaction Summary 不能为空")
    candidate = [{"role": "assistant", "content": "Conversation summary:\n" + summary}, *retained_tail]
    # ponytail: 字符数只作同口径长度比较；接真实模型时另用对应 tokenizer 验证窗口预算。
    if len(json.dumps(candidate, ensure_ascii=False)) >= prompt_chars:
...
def execute_workspace_tool_with_ledger(
    ...
    # 第五关 A。在副作用完成、Ledger 终态写入前调用 after_effect。
...
def main() -> None:
    ...
    if "--self-check" not in sys.argv:
        print("运行方式：python -B practice/workspace-agent/agent.py --self-check")
        return
    self_check()

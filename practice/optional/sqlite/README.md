# 选做专题：什么时候该从 JSONL 换成 SQLite

这是已完成的参考代码，不阻塞主线。先运行 `python -B practice/optional/sqlite/demo.py --checkpoint-d`，再按需要阅读下方函数和检查。想亲手重做时只修改当前一处规则；无需依次补写已经完成的函数。
...
打开 [`demo.py`](demo.py)，先只实现 `create_schema()`。
...
```bash
python -B practice/optional/sqlite/demo.py --checkpoint-a
```
...
```bash
python -B practice/optional/sqlite/demo.py --checkpoint-b
```
...
```bash
python -B practice/optional/sqlite/demo.py --checkpoint-c
```
...
```bash
python -B practice/optional/sqlite/demo.py --checkpoint-d
```

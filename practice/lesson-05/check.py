...
CORE = runpy.run_path(str(ROOT / "practice/workspace-agent/agent.py"))
...
def tool_message(call_id, path):
    call = CORE["fake_tool_call"](call_id, "read_file", {"path": path})
    return CORE["assistant_message_from_api"](
        CORE["fake_response"]("tool_calls", tool_calls=[call]).choices[0].message
    )


def self_check():
    chapter = ROOT / "chapters/05-上下文工程.md"
    ...
    with tempfile.TemporaryDirectory(prefix="agent-lesson05-check-") as folder:
        ...
        try:
            ...
            raise AssertionError("综合项目应拒绝更长的新视图")
        assert session_file.read_bytes() == before_failure

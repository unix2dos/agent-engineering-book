...
from grader import csv_quantity_total, safe_grade_config, safe_grade_csv_total, summarize_results


REPO_ROOT = Path(__file__).resolve().parents[2]
# 复用综合实践的循环、Tool 和 Ledger；综合项目同时提供工具说明与模型连接。
AGENT = SimpleNamespace(**runpy.run_path(str(
    REPO_ROOT / "practice/workspace-agent/agent.py"
)))
CONNECTION = runpy.run_path(str(REPO_ROOT / "practice/workspace-agent/client.py"))
TOOLS = [
...
RUNTIME_SHA256 = fingerprint({
    ...
    for relative in (
        "practice/lesson-09/run_trial.py",
        "practice/workspace-agent/agent.py",
        "practice/workspace-agent/client.py",
    )
})
GRADER_SHA256 = fingerprint(Path(__file__).with_name("grader.py").read_text(encoding="utf-8"))
try:

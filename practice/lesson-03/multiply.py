...
def self_check() -> None:
    ...
    offline = Mock()
    connect = Mock(side_effect=AssertionError("检查不能连接模型"))
    with patch.dict(globals(), self_check=offline, make_client=connect):
        main([])
        main(["--self-check"])
        assert offline.call_count == 2
        connect.assert_not_called()
        try:
            main(["--live"])
        except AssertionError as error:
            assert str(error) == "检查不能连接模型"
        else:
            raise AssertionError("--live 应进入真实模型连接分支")
        connect.assert_called_once()
    print("self-check passed (including default offline and explicit live entry)")

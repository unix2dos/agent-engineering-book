...
def check_listing(workspace):
    with patch.object(Path, "open", side_effect=AssertionError("列文件名不需要读取文件内容")):
        assert tools.list_files(workspace) == {
            "path": ".", "files": ["order_service.py", "server.log"], "next_offset": None,
        }, "只列本层普通文件，排好序，不包含子目录或软链接"
        assert tools.list_files(workspace, "logs") == {"path": "logs", "files": ["worker.log"], "next_offset": None}
        assert tools.list_files(workspace, "empty") == {"path": "empty", "files": [], "next_offset": None}
        for offset, names, next_offset in [
            (0, ["a.log", "b.log"], 2), (2, ["c.log", "d.log"], 4),
            (4, ["e.log"], None), (5, [], None), (8, [], None),
        ]:
            assert tools.list_files(workspace, "pages", offset, 2) == {
                "path": "pages", "files": names, "next_offset": next_offset,
            }
        assert tools.list_files(workspace, "pages", 0, 5)["next_offset"] is None
        default_page = tools.list_files(workspace, "many")
        assert len(default_page["files"]) == 20 and default_page["next_offset"] == 20
        largest_page = tools.list_files(workspace, "many", limit=100)
        assert len(largest_page["files"]) == 100 and largest_page["next_offset"] == 100
        assert tools.list_files(workspace, "many", 100, 100) == {
            "path": "many", "files": ["file_100.log"], "next_offset": None,
        }
        for path in ("../outside", str(workspace), "escape", "server.log", "missing", "", None, []):
            try:
                tools.list_files(workspace, path)
            except ValueError:
                pass
            else:
        ...
        for field, bad_values in {
            ...
            for value in bad_values:
                try:
                    tools.list_files(workspace, **{field: value})
                except ValueError:
                    pass
                else:
...
def check_loop(workspace, show_steps=False):
    ...
    answer = CORE["run_agent_loop"](
        client, "scripted", tools.TOOLS, "每页查看一个文件名，查完目录后读取 server.log。",
        execute,
    )
    ...
    assert json.loads(tools.execute_tool(workspace, default_call)) == tools.list_files(workspace)
    ...
    for name, args in (
        ...
        assert "error" in json.loads(tools.execute_tool(workspace, call))
    ...
    CORE["run_agent_loop"](corrected, "scripted", tools.TOOLS, "查看目录", lambda call: tools.execute_tool(workspace, call))
